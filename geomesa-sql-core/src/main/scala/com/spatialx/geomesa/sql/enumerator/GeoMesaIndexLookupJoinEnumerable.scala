...
/**
 * Produces an enumerator for performing index lookup join when enumerator was consumed
 */
class GeoMesaIndexLookupJoinEnumerable(
  ...
  predicate: Predicate2[AnyRef, AnyRef]) extends AbstractEnumerable[AnyRef] {
  override def enumerator(): Enumerator[AnyRef] = {
    new GeoMesaIndexLookupJoinEnumerator(cancelFlag, left, leftKeysSelector, right, rightQueryable, rightKeys,
